...
#### Scenario: Buildr Skill 感知完整更新意图
- **WHEN** 用户要求 Agent“更新 Buildr”且没有限定只更新 CLI
- **THEN** 产品内置 Buildr Skill 的 description 和正文 MUST 能让 Agent 识别该意图
- **AND** Buildr Skill MUST 引导 Agent 先运行 `buildr update`
- **AND** update 成功后 Agent MUST 重新解析当前 `buildr` 入口，再运行 `buildr sync <agent> --target <dir>`
- **AND** Agent MUST 使用 sync 的最终 doctor 结果判断完整更新是否完成

#### Scenario: Buildr Skill 感知只更新 CLI 意图
- **WHEN** 用户明确要求“只更新 Buildr”或不更新 workspace
- **THEN** Buildr Skill MUST 引导 Agent 只运行 `buildr update`
- **AND** Agent MUST NOT 追加 sync、runtime render 或 workspace doctor

#### Scenario: Buildr Skill 感知 workspace 同步意图
- **WHEN** 用户要求“同步 workspace”或等价意图
- **THEN** Buildr Skill MUST 引导 Agent 运行 `buildr sync <agent> --target <dir>`
- **AND** Agent MUST NOT 先更新 CLI

#### Scenario: CLI update 受阻时停止完整更新
- **WHEN** `buildr update` 返回 Git、registry、权限或来源决策点
- **THEN** Buildr Skill MUST 向用户说明阻塞事实和可执行选项
- **AND** Agent MUST NOT 使用旧 CLI 继续执行 sync
