...
Buildr MVP MUST 先通过 bootstrap guide、Buildr Skill、`init`、`project create`、`service create`、`doctor` 和按 Agent 类型选择的 runtime 动作完成 onboarding 闭环，不得要求先实现更高层 `buildr use` 入口。
...
- **THEN** Agent MUST 能通过 Buildr Skill 理解 Buildr workspace onboarding、项目创建、服务接入、诊断和 runtime 处理流程
- **AND** Buildr Skill MUST 复用 bootstrap guide 的 Agent-facing 核心流程
...
- **WHEN** Agent 完成 `buildr init`
- **THEN** bootstrap guide 和 Buildr Skill MUST 引导 Agent 先判断自身是否原生读取 `AGENTS.md`
