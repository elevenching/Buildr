...
Buildr 产品验证 MUST 自动检查薄 executable、command 唯一登记、模块依赖方向、npm runtime inventory 和直接文件写入边界，并 MUST 在模块回退为巨石、出现反向依赖、漏发运行时文件或绕过受管 mutation primitive 时失败。

#### Scenario: 维护者运行产品验证
- **WHEN** 维护者运行 CLI 架构专项检查或产品完整验证
- **THEN** verifier MUST 扫描全部发布 CLI runtime modules，而不是只检查 `tools/buildr` 单文件
- **AND** verifier MUST 对不符合分层或 mutation 白名单的实现返回非零状态和可定位文件/模块的诊断
