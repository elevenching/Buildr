...
| 运行测试、验证改动、判断开发验证是否完成、报告验证耗时、初始化/更新测试声明、推进测试能力成熟度，或实现任务到达验证/完成节点 | `buildr.task-verification/v1` selected provider；用户无需主动点名该能力 |
| 创建、定位、保留、迁移入口或清理 task worktree | `buildr.task-worktree-lifecycle/v1` selected provider |
...
- Project 表示业务、产品线、系统或长期工作单元；创建入口是 `buildr project create <project> --target <dir>`。
