...
Buildr 管理和投射的是 Agent 可发现、可选择、可使用的工作资产，不是当前任务的 context window。Agent 根据用户目标、修改范围、代码语义和 workspace 信息选择相关内容，形成任务上下文；Agent runtime adapter 不替 Agent 判断语义相关性。
