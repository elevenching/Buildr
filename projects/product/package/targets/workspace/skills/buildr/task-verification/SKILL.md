...
description: 用户要求测试、验证、耗时报告、初始化或更新测试声明、推进测试能力成熟度，或者实现型任务到达验证节点、Agent 准备判断或声称开发完成、Task Finish 需要核对或补齐 Candidate evidence 时使用；用户无需主动点名本 Skill。
---
...
单个普通实现任务完成后，只运行语法、类型、静态检查，或直接相关、低成本、环境就绪且已授权的 stable 能力。安全边界、不可逆迁移或用户明确要求的即时检查不受默认批量节奏限制。
