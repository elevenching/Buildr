...
5. 有 runtime token usage 时只把它作为辅助证据；没有精确 usage 时只做定性判断，不伪造 token 数量。安全检查、候选 tree 变化后的重验和契约要求产生的成本属于必要成本，不得为节省 token 削弱正确性。
6. 将结果分为“执行质量反馈”和“资产沉淀建议”。一次性失误、偶发环境状态和任务特有绕路可以进入质量反馈，但不自动晋升为长期资产。
...
优先引用最终 commit、相关 diff、归档 change、稳定文件路径或已有任务驾驶舱。若关键依据只能来自临时 session 节点，保留必要的自包含摘要并标记证据耐久性较弱，不声称以后可以恢复全部原始过程。
