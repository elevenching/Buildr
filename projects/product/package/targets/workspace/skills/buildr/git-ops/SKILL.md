---
name: git-ops
description: 用户表达提交、commit、推送、push、提交并推送、合并、merge、rebase、发布、release、删除分支、清理远端分支等单项 Git 意图，或 Git 操作存在授权、提交范围、amend、远端改写、分支删除等歧义时使用。用于统一 Git 协作约定和安全默认行为，而不是完整任务结束编排。
---
