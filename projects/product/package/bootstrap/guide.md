...
识别当前 Agent，并将 `<agent>` 固定为支持列表中对应的参数；当前支持 `claude-code` 和 `codex`。如果无法和支持列表对齐，停止 Buildr 操作，并请联系 Buildr 作者反馈该 Agent。
...
Agent runtime adapter 按“scope 祖先链 + scope 子树”发现和投射 `AGENTS.md`，不替 Agent 做语义决策。Agent 必须读取 enabled、required 且 installed 的 Rule；对 enabled、optional 且 installed 的 Rule，先检查 description，并在当前任务语义相关时于行动前读取正文。disabled 或 uninstalled Rule 不参与任务。
