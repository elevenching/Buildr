...
7. 使用 `node projects/product/tools/verification/release/release-notes.mjs <version> CHANGELOG.md` 生成目标版本的最终 GitHub Release notes 预览；目标章节缺失、重复或没有具体内容时视为发布阻塞。
...
1. 从不带 `v` 前缀的完整目标 package version 派生发布 task identity：task id 为 `release-<version>`，分支为 `tasks/release-<version>`，canonical worktree 为 `<workspace-root>/.worktrees/release-<version>`。同一版本已有分支和 worktree 时必须检查归属并复用，不创建第二个发布任务 identity；不得在 `main` 直接准备发布材料。
...
5. 从 workspace root 运行 `node projects/product/tools/verification/release/release-notes.mjs <version> CHANGELOG.md`，向维护者展示 workflow 将使用的最终 notes；提取失败或内容仍是笼统发布标题时继续维护 CHANGELOG，不进入候选验证。
...
8. 使用 `task-finish` 把准备改动集成并推送到 `dev`。
9. 检查本次发布范围是否涉及 Buildr CLI 入口或实现，包括 `buildr`、`tools/buildr`、CLI 使用的 `tools/*.mjs`、安装/卸载脚本或 npm CLI 映射。若涉及，必须从已经集成本次改动且会继续保留的 Product checkout 运行 `tools/install-buildr-cli`，再运行 `command -v buildr`、`buildr --help` 和 `buildr doctor --agent <agent> --target <workspace-root> --json`。安装或验证失败时停止发布准备；不得把本机入口留在即将清理的 task worktree，也不得要求维护者去其他 workspace 通过 Agent“更新 Buildr”来代替本地开发安装。该动作只刷新本机开发 CLI，不等同于 registry `buildr update`，也不触发其他 workspace 的 `buildr sync`。
10. 创建 `dev -> main` PR，等待必须的 CI 和 branch protection，通过后按仓库策略 squash merge 到 `main`。
11. PR 合入后使用 `node projects/product/tools/cli/application/release/bridge-main-to-dev.mjs --repo <workspace-root> --candidate-tree <tree>` 执行发布专用历史衔接。该工具必须先确认 `origin/main^{tree}` 和 `origin/dev^{tree}` 都与已验证 candidate tree 一致；`origin/main` 已是 `origin/dev` 祖先时 no-op，否则创建仅衔接历史、不改变 tree 的 merge commit，复核 tree 后普通 push `dev`。
12. 历史衔接中任一 tree identity 不一致、本地/远端 ref 已变、branch protection 或 push 拒绝时停止后续 tag 动作并报告实际状态。不得使用 force push、reset，也不得在 tree gate 失败时使用 `ours` 掩盖内容差异。
13. 确认 `main` 指向已验证内容，版本和发布材料一致，且远端 `dev` 已包含 squash `main` 历史。
14. 明确报告“准备完成，尚未创建 tag，尚未触发 npm 发布”，并在涉及 CLI 时同时报告本机开发入口的安装与验证结果，然后停止。
