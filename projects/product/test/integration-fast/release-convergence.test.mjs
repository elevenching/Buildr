...
function writeVersion(cwd, version, marker) {
  ...
  fs.writeFileSync(path.join(cwd, 'projects', 'product', 'package.json'), `${JSON.stringify({ name: '@buildr-ai/buildr', version })}\n`);
...
test('release convergence rejects stale version, tree and unintegrated release task', (t) => {
  ...
  const result = checkReleaseConvergence({ repo: data.work, version: '0.1.0-rc.6', candidateBase: data.candidateBase, candidateTree: `${data.candidateTree.slice(0, -1)}0`, stage: 'pre-main' });
