...
| `worktree create` | `buildr.worktree-create/v1` |

`buildr.worktree-create/v1` 的 `state` 为 `created`、`reused` 或 `blocked`，`treeChanged` 表示本次调用是否创建 checkout。`bootstrap.doctorBefore`、`bootstrap.sync` 和 `bootstrap.doctorAfter` 记录创建后的确定性环境准备；blocked 结果以非零退出并保留 `blocked` 与 `nextActions`，已经创建的 worktree 不会因 bootstrap 失败而自动删除。
