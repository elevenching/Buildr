...
export async function renderChangeDetail({ root, api, onWorkspace, openAgentAction, params }) {
  const { projectCode, changeRef } = params;
  root.innerHTML = `<section class="page-header change-detail-header"><a class="back-link" href="/changes" data-route>← 返回变更目录</a><div class="page-header-row"><div><p class="eyebrow">变更（Change）</p><h1 id="change-detail-name">正在读取…</h1><p id="change-detail-copy" class="page-copy">读取 Change 契约与任务状态。</p></div><div class="panel-actions"><button id="continue-change" class="button primary" type="button">继续推进</button><button id="review-change" class="button secondary" type="button">交给 Agent 审查</button></div></div></section><section class="metric-grid change-metrics"><article class="metric-card identity-card"><span>Change ID</span><strong id="change-detail-code">—</strong><small id="change-detail-project">—</small></article><article class="metric-card"><span>生命周期</span><strong id="change-detail-lifecycle">—</strong><small>来自实际目录位置</small></article><article class="metric-card"><span>任务进度</span><strong id="change-detail-progress">—</strong><small id="change-detail-updated">—</small></article></section><section class="panel"><div class="panel-heading"><div><p class="eyebrow">标准产物</p><h2>OpenSpec artifacts</h2></div><span class="state">只读</span></div><div id="change-artifacts" class="artifact-list"></div></section>`;
  try {
    ...
    document.getElementById('change-detail-name').textContent = change.name; document.getElementById('change-detail-copy').textContent = `查看 ${change.project.name} 中的真实 OpenSpec Change；页面不直接修改文件。`;
    ...
    const container = document.getElementById('change-artifacts'); container.append(artifactPanel('提案（proposal）', change.artifacts.proposal), artifactPanel('设计（design）', change.artifacts.design));
    for (const spec of change.artifacts.specs) container.append(artifactPanel(`规格（spec）· ${spec.capability}`, spec));
    container.append(artifactPanel('任务（tasks）', change.artifacts.tasks));
  ...
  } catch (error) { root.innerHTML = `<section class="page-header"><p class="eyebrow">变更（Change）</p><h1>变更不存在</h1><p class="page-copy"></p></section><a class="button secondary" href="/changes" data-route>返回变更目录</a>`; root.querySelector('.page-copy').textContent = error.message; }
}
