...
export async function renderChanges({ root, api, onWorkspace, openAgentAction }) {
  root.innerHTML = `
    <section class="page-header page-header-row">
      <div><p class="eyebrow">变更（Change）</p><h1>变更</h1><p class="page-copy">查看各项目的 OpenSpec Change。创建、继续和审查均生成 Agent 指令，Buildr 不直接改写生命周期。</p></div>
      <button id="create-change-button" class="button primary" type="button">创建变更</button>
    </section>
    <section class="panel change-filters">
    ...
    <section class="panel changes-panel">
      <div class="panel-heading"><div><p class="eyebrow">变更目录</p><h2>OpenSpec Change</h2></div><span id="changes-state" class="state">正在读取</span></div>
