...
export async function renderServiceDetail({ root, api, onWorkspace, onBreadcrumb, params }) {
  ...
  root.innerHTML = `
    ...
    <section class="detail-facts-section" aria-label="服务详情"><dl class="read-facts detail-facts"><div><dt>服务代码</dt><dd id="service-code">—</dd></div><div><dt>服务说明</dt><dd id="service-detail-description">—</dd></div><div><dt>所属项目</dt><dd id="service-detail-project">—</dd></div><div><dt>服务类型</dt><dd id="service-detail-type">—</dd></div><div><dt>来源类型</dt><dd id="service-detail-source">—</dd></div><div><dt>来源路径</dt><dd id="service-path">—</dd></div></dl><details class="technical-details"><summary>技术信息</summary><dl class="read-facts technical-facts"><div><dt>服务 ID</dt><dd id="service-id">—</dd></div><div><dt>项目代码</dt><dd id="service-project-code">—</dd></div><div><dt>修订版本</dt><dd id="service-revision">—</dd></div><div><dt>集成分支</dt><dd id="service-integration-branch">—</dd></div><div><dt>当前分支</dt><dd id="service-current-branch">—</dd></div><div><dt>Git 状态</dt><dd id="service-git-state">—</dd></div></dl><div id="service-findings" class="findings"></div></details></section>`;
  try {
    ...
    text('service-detail-name', service.name); text('service-detail-description', service.description || '尚未填写服务说明。'); text('service-detail-project', `${data.project?.name || projectCode}（${projectCode}）`); text('service-detail-type', ({ backend: '后端', frontend: '前端', application: '应用', library: '库', tool: '工具' })[service.type] || service.type); text('service-detail-source', service.source.type === 'git' ? 'Git 仓库' : service.source.type === 'workspace' ? '当前工作空间' : '本地路径'); text('service-code', service.code); text('service-path', service.source.path);
    text('service-id', service.id || '迁移后生成'); text('service-project-code', projectCode); text('service-revision', data.revision); text('service-integration-branch', service.source.git?.integrationBranch || '不适用'); text('service-current-branch', data.observed?.currentBranch || (service.source.type === 'git' ? '暂时无法读取' : '不适用')); text('service-git-state', data.observed ? `${data.observed.dirty ? '有未提交变化' : '干净'} · 领先 ${data.observed.ahead ?? '—'} / 落后 ${data.observed.behind ?? '—'}` : '不适用');
