...
export function resolveRenderSkills(repoRoot, scope, runtime) {
  ...
  if (projectRoot) return resolveSkills(organizationRoot, projectRoot, { runtime });
  const groups = [{ scope: '.', skills: resolveSkills(organizationRoot, null, { runtime }) }];
  ...
  return groups.flatMap((group) => group.skills.map((skill) => ({ ...skill, declaredScope: group.scope })));
}
