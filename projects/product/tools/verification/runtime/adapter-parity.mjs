...
const adapterOrder = ['cursor', 'qoder', 'trae', 'trae-work', 'workbuddy', 'claude-code', 'codex'];
for (const agent of adapterOrder) {
  ...
  if (agent === 'trae-work') assert.match(check.stdout, /runtime\.trae_work_rules_import_unverified|Rules import and reference traversal/);
...
assert.ok(fs.readFileSync(path.join(workspace, 'projects', 'scope-alpha', 'services', 'api', '.trae', 'rules', 'buildr.md'), 'utf8').includes('alwaysApply: true'));
assert.ok(fs.readFileSync(path.join(workspace, 'projects', 'scope-alpha', 'services', 'api', '.trae', 'rules', 'buildr.md'), 'utf8').includes('API_MARKER'));
assert.ok(fs.readFileSync(path.join(workspace, 'projects', 'scope-alpha', 'services', 'web', '.trae', 'rules', 'buildr.md'), 'utf8').includes('WEB_MARKER'));
assert.ok(fs.readFileSync(path.join(workspace, 'CLAUDE.local.md'), 'utf8').includes('projects/scope-alpha/services/api/AGENTS.md'));
...
assert.ok(fs.existsSync(path.join(workspace, '.trae', 'skills', 'buildr', 'SKILL.md')));
...
for (const agent of adapterOrder) run(['render', agent, '--scope', '.', '--target', workspace]);
...
for (const agent of ['cursor', 'qoder', 'trae', 'trae-work', 'workbuddy']) {
...
assert.equal(fs.existsSync(path.join(rulesOrphanWorkspace, '.trae', 'rules', 'buildr.md')), false);
assert.equal(fs.existsSync(path.join(rulesOrphanWorkspace, 'CLAUDE.local.md')), false);
