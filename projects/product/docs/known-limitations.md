...
- Agent 没有统一 API 枚举已加载的 admin/system/plugin Skills。adapter 会报告 `partial` inventory evidence；Buildr 阻止已观察到的同名冲突，但不宣称已证明 Agent 全局唯一。首版不提供自动 adopt/transfer，外部资产必须重命名、显式移除/禁用或保持现场。
...
- `@buildr-ai/buildr@0.1.0-rc.5` 通过 GitHub trusted publisher 发布，`next` 指向该 RC；`latest` 仍可能指向历史 prerelease，它不代表稳定版。稳定版 `0.1.0` 尚未发布，公开试用应显式安装 `@next`。`0.1.0-rc.4` 因发布范围错误已弃用。
