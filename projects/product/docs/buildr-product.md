...
- `task-verification` 是 `buildr.task-verification/v1` 的默认 provider，对 Buildr 自举项目和用户 workspace 提供通用的三级验证协议：单任务最小反馈、任务组受影响范围验证、最终候选完整验证。它既响应用户直接验证意图，也在实现任务到达验证节点、Agent 准备声称完成或 Task Finish 需要 Candidate evidence 时自动参与，用户无需点名 Skill。具体命令由当前 workspace 或 Project 定义；provider 负责执行、候选 identity、真实 wall-clock、用户报告和落盘 evidence 生命周期，不把 Buildr 产品验证入口硬编码为所有项目的固定命令。provider operation 分为 `inspect`、`execute` 和 `cleanup`，只有 `execute` 启动验证命令；加载 provider、核对 summary 或清理 evidence 都不计作再次执行验证。
