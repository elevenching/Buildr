...
- 复杂、长期、跨阶段或存在交叉依赖的任务可以由 `task-cockpit` Skill 在 Project `openspec/knowledge/task-cockpits/yyyy-MM-dd-<task-id>.html` 维护稳定的只读 HTML 驾驶舱。驾驶舱覆盖整个任务并可关联多个 change；首页优先用普通语言展示目标、当前结论、阶段、下一步和阻塞，推进、方案与技术细节逐层后置。用户通过 Agent 对话参与，Agent 核实事实后单向更新页面并在关键进展回复中提供入口。
...
- `task-asset-review` 是 optional 内置任务资产审查 Skill。用户明确要求复盘或沉淀任务成果时直接使用；`task-finish` 则在任务语义、候选 tree、验证证据和 contract sidebar 结论可确认后，只根据当前上下文执行不调用工具的轻量资格判断，命中工作边界纠正、假设推翻、有效失败根因、无效重复、token 浪费、新长期约束或可复用流程等强信号时才条件调用完整审查。审查使用当前 session 可观察的用户纠正、计划状态、工具输出、失败重试、关键决策、subagent 报告、diff 和验证结果，先反馈执行质量，再筛选 Rule 或 Skill 候选并生成可独立引用的证据胶囊；OpenSpec 只作为任务契约和证据。该能力不读取隐藏推理，不保存完整对话或任务轨迹，不引入 Hook、daemon、watcher 或事件总线，不把“收尾”扩大为自动写入组织资产的授权。
