...
- `task-worktree` 只通过 `buildr.task-worktree-lifecycle/v1` 管理 task checkout placement、retention、cleanup、入口迁移和 `treeChanged` 证据；它不拥有验证命令选择、三级执行或验证报告政策。验证可以在有 worktree、当前分支或非 Git 候选中独立发生。
