...
- `task-verification` 是 `buildr.task-verification/v1` 的默认 provider，对 Buildr 自举项目和用户 workspace 提供通用的三级验证协议：单任务最小反馈、任务组受影响范围验证、最终候选完整验证。Project 可以通过可选 `verification.yml` 声明任意测试能力、成熟度、适用阶段、覆盖、环境、副作用、授权和门禁；没有声明时继续读取 AGENTS、POM、项目文档和已有入口，不增加失败、阻塞或新环境启动。用户说“初始化测试声明”时，Agent 生成 augment/discovered 候选；后续在任务中增量发现、试运行并建议 `discovered → trial → stable`，但 stable/required 仍需团队确认。provider 既响应直接验证和声明演进意图，也在实现任务到达验证节点、Agent 准备声称完成或 Task Finish 需要 Candidate evidence 时自动参与。它按 Minimal/Affected/Candidate 动态选择任意能力集合，披露选中、跳过、阻塞、授权和 Candidate 完整性，不把鲜肉或其他团队分层写进通用 Skill。provider operation 仍分为 `inspect`、`execute` 和 `cleanup`，只有 `execute` 启动验证命令；加载 provider、核对 summary 或清理 evidence 都不计作再次执行验证。
- `task-worktree` 通过 `buildr.task-worktree-lifecycle/v1` 管理 task checkout placement、确定性创建后环境 bootstrap、retention、cleanup、入口迁移和 `treeChanged` 证据；Agent 仍负责任务理解与 branch/start point 决策。它不拥有验证命令选择、三级执行或验证报告政策。验证可以在有 worktree、当前分支或非 Git 候选中独立发生。
- Buildr 产品完整验证绑定最终候选 Git tree；commit、相同 tree 集成、push 和 worktree 清理不触发主开发分支重复 E2E，tree 改变后才在集成前重验受影响部分。
- `task-finish` 是 Buildr 自有的任务收尾编排 Skill；用户在 task worktree 中明确说“收尾”时，它组合当前已安装 Component 贡献的工作流说明、外部 OpenSpec、selected Task Verification、Git Ops 和 Task Worktree providers，复用或补齐 Candidate evidence 后完成常规归档、集成、推送和本地清理。收尾同时记录实现候选身份（`implementationCandidateIdentity`）和交付 tree 身份（`deliveryTreeIdentity`）：`same-content` 与可归因的 `closeout-metadata-only` transition 只 inspect/reuse，不再次启动 Candidate；只有 `implementation-changed` 或无法证明仅为收尾元数据时才失效旧证据并 execute。Buildr Product 还把同一会话内刚成功的最终 Candidate task 唯一 `- [ ]` → `- [x]` 变化定义为 `closeout-metadata-only` 的 `verification-result-metadata-only` subtype：Candidate evidence 仍绑定 source implementation identity，consumer 另存 source/target identity、change/task identity 与精确 marker transition；它不宣称 Candidate 直接验证 target delivery tree。额外编辑、任务歧义或跨会话缺证据一律 fail closed。OpenSpec strict、contract guard 和 diff check 是 closeout delta checks，不伪装成 Candidate executor 调用。所有 consumer 使用完 transient evidence 后，Task Finish 请求 verification provider 清理精确 run；caller-managed/CI artifact 不自动删除，worktree lifecycle 不承担验证证据清理。最终报告复用验证摘要并披露两类 identity、transition class/subtype、provider operations、executor 调用计数及 retention/cleanup status，不把已删除的临时路径描述为长期引用；OpenSpec Component 已安装时贡献同步前后契约门禁，卸载后通用 Skill 不保留相关命令。
- `task-asset-review` 是 optional 内置任务资产审查 Skill。用户明确要求复盘或沉淀任务成果时直接使用；`task-finish` 则在任务语义、候选 tree、验证证据和 contract sidebar 结论可确认后，只根据当前上下文执行不调用工具的轻量资格判断，命中强信号时才条件调用完整审查。审查使用当前 session 可观察节点、diff 和验证结果，并按需核对候选 Skill 的正文、metadata、模板、脚本、manifest、runtime 投射与真实产物，输出完整覆盖、部分覆盖、存在冲突或尚无资产；runtime 仅作同步证据。它先反馈执行质量，再筛选 Rule 或 Skill 候选并生成证据胶囊；OpenSpec 只作为任务契约和证据。该能力不读取隐藏推理，不保存完整对话或任务轨迹，不引入 Hook、daemon、watcher 或事件总线，不把“收尾”扩大为自动写入组织资产的授权。
