...
- 复杂、长期、跨批次或存在交叉依赖的任务可以由 `task-cockpit` Skill 在 Project `openspec/knowledge/task-cockpits/yyyy-MM-dd-<task-id>.html` 维护稳定的只读 HTML 任务驾驶舱（任务看板）；两种名称指向同一 artifact。看板覆盖整个任务并至少关联一个真实 OpenSpec change，以可独立交付批次和依赖池组织进度；首页优先用普通语言展示目标、当前结论、当前批次、下一步和阻塞，change 关联、业务/技术方案与已完成技术细节逐层后置。用户通过 Agent 对话参与，Agent 核实事实后单向更新页面并在关键进展回复中提供入口。
