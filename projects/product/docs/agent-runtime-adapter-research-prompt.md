...
- 不要把“安装包中出现某路径”“目录存在”“文件已生成”当成 Agent 会发现它；必须说明该路径的实际用途。
...
对每个观察到的 Skills 路径标明用途：project discovery、user discovery、builtin、editor/UI、sandbox writable 或 unknown。projectRoot 必须由当前会话真实发现测试 Skill、产品文档明确声明或 discovery 源码直接证明；只有字符串命中时保持 unknown。
...

{
  ...
  "skills": {
    ...
    "userRoot": null,
    "otherObservedPaths": [
      { "path": "", "purpose": "project_discovery|user_discovery|builtin|editor_ui|sandbox_writable|unknown", "evidence": "" }
    ],
