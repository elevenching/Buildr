...
- [x] 建立 GitHub Actions 最小 CI，运行 `projects/product/scripts/verify-buildr-product`。
...
2. 根 `CHANGELOG.md` 必须包含唯一的 `## <version> - <YYYY-MM-DD>` 章节和非空正文。冻结候选前从 workspace root 运行 `node projects/product/scripts/release/release-notes.mjs <version> CHANGELOG.md`，预览 GitHub Release 将使用的最终 Markdown。
