...
所有表中 Skills root 同时具有 workspace 与 user 两个 destination：前者相对 `--target` 工作目录，后者相对当前用户目录。adapter 还声明可观测 discovery roots、`complete|partial` inventory evidence、未知 admin/system/plugin 边界和 activation；当前 adapters 的内部来源均只能部分观察，因此成功 render 只证明可观测范围内没有冲突。Buildr 用 `buildr.skill-projection/v2` receipt 记录 destination、asset/source identity、source workspace、source/render digest 和文件 inventory。外部等价、其他 owner 或同名异内容均不由 `--replace` 接管。
