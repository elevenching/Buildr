...
所有表中 Skills root 都接收相同的 filesystem Skills primitive：本地作者型和 package Skill 会完整投射 `SKILL.md`、`agents/`、`assets/`、`examples/`、`references/`、`scripts/`、`templates/` 中的普通文件，并保留随附文件字节和 owner executable 状态。完整投射表示保留源 inventory，不表示每个 adapter 都消费所有 vendor extension。Buildr 为每个 adapter/Skill 写入独立 projection receipt，用于幂等检查和安全 stale/orphan 清理；目标被修改或含未知用户文件时停写。远端 `skill-url` 仍只投射单个 `SKILL.md`。
