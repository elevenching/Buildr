...
Buildr 是 AI 时代的工作资产管理工具。它把个人和组织的工作方法与工作事实，沉淀为 Agent 可发现、可选择、可使用的工作资产。

你指挥，Agent 构建；资产归你，Agent 可换。

你继续和 Agent 对话；Agent 通过 Buildr 管理工作资产，并按任务选择需要的内容。

Buildr 不是另一个 Agent，也不和 Agent 竞争。它让你正在使用的 Agent 基于持续积累的事实和方法完成更多工作，工作越做越稳、越做越好。

## 三个核心价值

### 1. 从产品到发布，Agent 全链路打通

Buildr 让一个需求可以在同一个 Agent 窗口里，持续基于同一套工作资产，从 PRD、设计、开发、测试一路推进到 CI/CD 和上线。

如果每一步都要重新解释背景、搬运文档，Agent 就不可能稳定完成整件事。Buildr 让 Agent 完成当前阶段后，直接基于已有事实和方法进入下一阶段。

**Buildr 自己就是这么开发的**：从讨论、OpenSpec 提案到开发实现、测试，再到 Git、GitHub Actions 和 npm 发布，全在一个 Agent 窗口里完成。
...
工作资产不应该绑定在某个 Agent 工具里。Buildr 把它们保存在独立的 Workspace 中，由个人或组织掌控，再为不同 Agent 生成各自需要的原生入口。换的是 Agent，不是你积累的东西。
